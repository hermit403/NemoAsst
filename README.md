...
**推荐**：在确保项目已安装并设置好所有API Key后，Windows环境下使用start.bat即可一键启动本项目。

如果NAT Server报错，请检查本地Docker Desktop是否已启动。

同时，项目也提供了dockerfile，方便使用docker一键部署开发环境

功能示例（推荐在设置中勾选Enable Intermediate Steps以展示调用过程）：

1. 用nmap快速扫描一下本地开放端口
![1-1](docs/image1-1.png)
![1-2](docs/image1-2.png)

2. 查一下这个网站的证书：www.baidu.com
![2](docs/image2.png)

## In Future...

- 接入更多MCP，例如和MySQL、SQLite、Redis等数据库相关的MCP，便于运维本地开发环境
- 优化Prompt，得到更好的使用效果
- 修改前端UI，更符合项目展示需求
- 完成任务后图表展示
- 持久化记忆，避免在同一问题上重复思考
